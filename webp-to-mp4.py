...
def get_webp_delay_first_frame(input_file_path):
    try:
        # Exécuter identify pour obtenir des informations sur l'animation WebP
        ...
        if result.returncode == 0:
            ...
            if total_frames > 0:
                ...
                print("Aucune frame trouvée dans le fichier WebP.")
        else:
...
def convert_webp_to_mp4(input_file_path, output_file_path):
    ...
    delay_first_frame = get_webp_delay_first_frame(input_file_path)
...
try:
    files = [filename for filename in os.listdir(input_directory) if filename.endswith(".webp")]
